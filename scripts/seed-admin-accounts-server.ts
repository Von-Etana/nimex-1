...
async function createPermissions() {
    ...
    for (const permission of PERMISSIONS) {
        try {
            const permRef = await db.collection(COLLECTIONS.ADMIN_PERMISSIONS).add({
...
async function createRoles() {
    ...
    for (const role of ROLES) {
        try {
            ...
            const roleRef = await db.collection(COLLECTIONS.ADMIN_ROLES).add({
                name: role.name,
            ...
            for (const permissionName of role.permissions) {
                ...
                if (permissionId) {
                    await db.collection(COLLECTIONS.ROLE_PERMISSIONS).add({
                        role_id: roleRef.id,
    ...
    try {
        ...
        try {
            ...
            if (error.code === 'auth/email-already-exists') {
                console.log(`  ℹ User already exists, fetching...`);
        ...
        if (roleId) {
            await db.collection(COLLECTIONS.ADMIN_ROLE_ASSIGNMENTS).add({
                user_id: uid,
                role_id: roleId,
...
async function seed() {
    console.log('🚀 Starting Admin Setup (Server-side)...\n');
    console.log('═══════════════════════════════════════════════════\n');
