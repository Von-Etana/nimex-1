...
export const SupportScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ticketSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
          logger.info('Support ticket updated', payload);
      ...
      .on(
        ...
        (payload) => {
          logger.info('New support ticket created', payload);
          debouncedLoadTickets();
        }
      )
      .subscribe();
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      if (attachments.length > 0) {
        const uploadResult = await uploadSupportAttachments(attachments, user?.id || '');
