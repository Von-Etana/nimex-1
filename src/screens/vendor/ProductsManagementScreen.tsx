...
export const ProductsManagementScreen: React.FC = () => {
  ...
  const loadProducts = async () => {
    try {
      ...
      // Helper function to fetch products with fallback for missing index
      const fetchProductsWithFallback = async (vendorId: string): Promise<Product[]> => {
        try {
          // Try ordered query first (requires composite index)
          return await FirestoreService.getDocuments<Product>(COLLECTIONS.PRODUCTS, {
            filters: [{ field: 'vendor_id', operator: '==', value: vendorId }],
            orderByField: 'created_at',
            orderByDirection: 'desc'
          });
        } catch (error: any) {
          // If index error, fall back to unordered query and sort client-side
          if (error?.message?.includes('index') || error?.code === 'failed-precondition') {
            console.warn('Index not available, using fallback query without ordering');
            const unorderedProducts = await FirestoreService.getDocuments<Product>(COLLECTIONS.PRODUCTS, {
              filters: [{ field: 'vendor_id', operator: '==', value: vendorId }]
            });
            // Sort client-side by created_at descending
            return unorderedProducts.sort((a, b) => {
      ...
      productsData = await fetchProductsWithFallback(user.uid);
      ...
      if (productsData.length === 0) {
        ...
        if (!vendorDoc) {
          ...
          if (vendors.length > 0 && vendors[0].id !== user.uid) {
            ...
            productsData = await fetchProductsWithFallback(vendors[0].id);
