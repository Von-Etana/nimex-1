...
export class FirestoreService {
    ...
    static async getCount(
        ...
        try {
            ...
            // Fallback to client-side count if server-side fails (e.g. older SDK)
            try {
     ...
     * Handles chunking for 'in' query limitations (max 10)
     */
    static async getDocumentsByIds<T = DocumentData>(
        collectionName: string,
        ids: string[]
    ): Promise<T[]> {
        ...
        try {
            ...
            logger.error(`Error getting documents by IDs from ${collectionName}`, error);
            throw error;
        }
    }
}
